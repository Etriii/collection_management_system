...
function handleCreateStudent() {
    ...
    set: newStudent.value.set,
    status: newStudent.value.status,
    };

    testData.value.push(student);
...
<template>
    ...
    <div class="h-screen bg-gray-100 font-sans antialiased text-gray-800 flex flex-col">
        <div class="container mx-auto px-4  flex-grow flex flex-col">
            <div class="bg-white rounded-2xl shadow-lg p-6 md:p-8 flex flex-col h-full">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                    <div>
                        <h2 class="text-3xl font-bold text-gray-900 leading-tight">Student Records</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 sticky top-0">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Student ID</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Full Name</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Program/Year/Set</th>
                ...
                <div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-200 flex-shrink-0">
                    <div class="text-sm text-gray-600">
                        Showing <span class="font-medium">{{ (currentPage - 1) * perPage + 1 }}</span> to <span class="font-medium">{{ Math.min(currentPage * perPage, filteredStudents.length) }}</span>
        ...
        <div
            v-if="isCreateStudentDialogOpen"
        ...
        >
            <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xl transform transition-transform duration-300 scale-95 opacity-0"
                :class="{'scale-100 opacity-100': isCreateStudentDialogOpen}">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold text-gray-800">Create New Student</h2>
                    <button @click="isCreateStudentDialogOpen = false" class="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <form class="space-y-5" @submit.prevent="handleCreateStudent">
                    <div>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button
                            ...
                            @click="isCreateStudentDialogOpen = false"
                        >
                            Cancel
                        </button>
                        <button
